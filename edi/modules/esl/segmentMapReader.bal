...
public function readSegmentDefinitions(string[] lines) returns map<edi:EDISegSchema>|error {

    map<int> fieldNameCount = {};
    map<edi:EDIFieldSchema> fieldMaps = check readFieldMetadata(lines);
    map<edi:EDISegSchema> segMaps = {};
    edi:EDISegSchema currentSegMap = {code: "NA", tag: "NA"};
    ...
    foreach string currentline in lines {
        ...
        if line.startsWith("- id:") {
            ...
            string codeTag = deriveTag(fieldNameCount, "code");
            edi:EDIFieldSchema codeField = {tag: codeTag, required: true};
            currentSegMap.fields.push(codeField);
            ...
            if fieldCode is string {
                edi:EDIFieldSchema? fieldMap = fieldMaps[fieldCode];
                if fieldMap is edi:EDIFieldSchema {
                    edi:EDIFieldSchema fmapcopy = fieldMap.clone();
...
function readFieldMetadata(string[] lines) returns map<edi:EDIFieldSchema>|error {
    log:printDebug("Reading single value feild definitions...");
    boolean fieldsSection = false;
    map<edi:EDIFieldSchema> fieldMaps = {};
    foreach string currentLine in lines {
        string line = currentLine.trim();
        if line.startsWith("elements:") {
            ...
            if fieldCode is string && fieldTag is string {
                edi:EDIFieldSchema fieldMap = {tag: getBalCompatibleName(fieldTag)};
    ...
    while i < lines.length() {
        ...
        if line.startsWith("composites:") {
            ...
            edi:EDIFieldSchema componentFieldDefinition = {tag: componentCode, dataType: edi:COMPOSITE};
            ...
            foreach int j in i+1...lines.length() {
                ...
                if componentLine.startsWith("name") {
                    ...
                    if fieldCode is string {
                        edi:EDIFieldSchema? fieldDefinition = fieldMaps[fieldCode];
                        if fieldDefinition is edi:EDIFieldSchema { 
                            edi:EDIComponentSchema componentDefinition = 
