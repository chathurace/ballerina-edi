...
function generateRecordForSegment(edi:EDISegMapping segmap, GenContext context) returns BalRecord {
    ...
    foreach edi:EDIElementMapping emap in segmap.elements {
...
function generateRecordForComposite(edi:EDIElementMapping emap, GenContext context) returns BalRecord {
    string cTypeName = generateTypeName(emap.tag, context);
    BalRecord crec = new(cTypeName);
    foreach edi:EDISubelementMapping submap in emap.subelements {
